...
target = liblo.Address("192.168.1.108", 1234) # the raspberry pi server
...
def callback(message, time_stamp):
    ...
    if (message[1] >= 60) and  (message[1] <= 67): # only allow right messages in for now
        properSolenoid = message[1] - 60 + 1 # 60 is the first note, 1 is the first solenoid
        if message[0] == 144: # noteOn
            liblo.send(target, "/foo", ('i', properSolenoid) , ('i', message[2])) # all the way on for first solenoid
